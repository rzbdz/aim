...
not a hope. Read the writer first: `cmd_room_say` (bin/aim:2741-2745) takes
...
first (two writers claim `r0004`, so `r0005` is never allocated) and the body
set goes red with it. Replace the `append_chained` call (bin/aim:414) with a
plain `open(path, "a")` and the `prev`/hash checks go red while the count stays
12 -- which is exactly the "well-formed and one fact short" shape the count-only
check would have missed.

One thing the card does not say and the code does: the room must be *published*
before 12 peers may write to it. `_load_room_or_die` (bin/aim:2560) refuses a
...
exit 2 with "REFUSED: room 'r' in ch is a draft"). So the fixture publishes the
room first -- one recorded act, which is the deliberate-open design/06 §2
specifies. The refusal is the barrier working, not a bug, and it is a *setup*
step here rather than a check: T-0198/T-0219's files own the gate's behaviour.
...
def canonical(obj):
    """bin/aim:155-157, copied rather than imported: the digest only means
    ...
    `AIM_ROOT` read (bin/aim:31) into this process. `sort_keys` and the compact
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    # (bin/aim:856) -- a refusal that would look like a concurrency failure.
    ...
    check(f"all {WRITERS} concurrent writers exit 0",
          ...
          f"return codes {rcs}: " + "; ".join(
              (p.stderr or "").strip().splitlines()[0]
              for p in procs if p.returncode) [:400])
    ...
    broken_prev = [(i, r.get("id"), r.get("prev"), recs[i - 2].get("hash"))
                   ...
                   if r.get("prev") != (recs[i - 2]["hash"] if i > 1 else "genesis")]
    ...
    bad_hash = [(r.get("id"), r.get("hash")) for r in recs
                ...
                != r.get("hash")]
    check("every record's hash is the hash of its own content (bin/aim:414's append_chained)",
...
print("failure T-0026 names. The lock in bin/aim:2740 and the chain in bin/aim:414 are")
